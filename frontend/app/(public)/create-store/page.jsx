...
import { setStoreInfo } from "@/lib/features/user/userSlice"
...
export default function CreateStore() {
    ...
    const onChangeHandler = (e) => {
        setStoreInfoState({ ...storeInfo, [e.target.name]: e.target.value })
    }

    const fetchSellerStatus = async () => {
        try {
            const data = await api.stores.getStatus();
            if (data.registered) {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            const data = await api.stores.create(formData);
            setAlreadySubmitted(true);
            setStatus('pending');
            setMessage('Your store is pending verification. Please wait for admin approval.');
            toast.success('Store details submitted successfully!');
        } catch (error) {
            console.error(error);
            toast.error(error.message || 'Failed to submit store details');
            throw error;
        }
    }
                ...
                <div className="mx-6 min-h-[70vh] my-16">
                    <form onSubmit={e => toast.promise(onSubmitHandler(e), { loading: "Submitting data..." })} className="max-w-7xl mx-auto flex flex-col items-start gap-3 text-slate-500">
                        ...
                        <div>
                            ...
                            <p className="max-w-lg">To become a seller on ReDragon, submit your store details for review. Your store will be activated after admin verification.</p>
                        </div>
                        ...
                        <label className="mt-10 cursor-pointer">
                            ...
                            <input type="file" accept="image/*" onChange={(e) => setStoreInfoState({ ...storeInfo, image: e.target.files[0] })} hidden />
                        </label>
