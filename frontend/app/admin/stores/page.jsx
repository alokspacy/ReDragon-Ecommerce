...
export default function AdminStores() {
    ...
    const fetchStores = async () => {
        try {
            const data = await api.stores.getAllAdmin();
            // Show approved or rejected stores
            const activeOrRejectedStores = data.filter(store => store.status !== 'pending');
            setStores(activeOrRejectedStores);
        } catch (error) {
    ...
    const toggleIsActive = async (storeId) => {
        ...
        try {
            const updated = await api.stores.approve(storeId, newStatus);
            setStores(stores.map(s => s.id === storeId ? { ...s, status: updated.status, isActive: updated.isActive } : s));
