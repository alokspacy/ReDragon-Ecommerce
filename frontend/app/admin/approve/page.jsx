...
export default function AdminApprove() {
    ...
    const fetchStores = async () => {
        try {
            ...
            console.error("Failed to load stores for approval:", error);
    ...
    const handleApprove = async ({ storeId, status }) => {
        try {
            await api.stores.approve(storeId, status);
            setStores(stores.filter(store => store.id !== storeId));
            toast.success(`Store registration ${status} successfully!`);
        } catch (error) {
            console.error(error);
            toast.error(error.message || `Failed to update store status to ${status}`);
            throw error;
        }
    }
